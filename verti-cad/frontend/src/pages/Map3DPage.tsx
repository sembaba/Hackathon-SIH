...
export const Map3DPage: React.FC<Map3DPageProps> = ({ searchQuery, onClearSearch }) => {
  ...
  const [isCameraToolsOpen, setIsCameraToolsOpen] = useState<boolean>(true);
    ...
    <div ref={mapContainerRef} className={`map-page-container ${isFullPage ? 'fullpage-mode' : ''}`}>
      ...
      <div className="map-left-dock">
        ...
        <div className="camera-mode-panel">
          <div
            className="camera-mode-header"
            onClick={() => setIsCameraToolsOpen(!isCameraToolsOpen)}
            title={isCameraToolsOpen ? 'Click to collapse tools' : 'Click to expand tools'}
          >
            <div className="camera-mode-label">
              <Globe size={13} />
              <span>3D CAMERA &amp; TOOLS</span>
            </div>
            <button
              type="button"
              className="camera-mode-toggle-btn"
              aria-label={isCameraToolsOpen ? 'Collapse' : 'Expand'}
            >
              {isCameraToolsOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            </button>
          </div>

          {isCameraToolsOpen && (
            <div className="camera-mode-body">
      ...
      {/* ─── GIS Layer Menu ──────────────────────────────── */}
      {showLayerMenu && (
        <div className="map-floating-panel" style={{ top: '3.5rem', right: '1rem', width: '235px', padding: '0.85rem', zIndex: 35 }}>
          ...
          {[
            ...
            { key: 'roads', label: 'Road Networks & Traffic' },
            { key: 'transport_corridor', label: 'Elevated 3D Corridor & Transit' },
